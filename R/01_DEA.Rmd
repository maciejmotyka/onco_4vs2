...
recount2_ensg_ids <- recount2_counts %>% dplyr::select(gene_id)
...
protein_coding <- biomaRt::getBM(attributes=c("ensembl_gene_id",
                                              "entrezgene_id",
...
## Clean up
```{r}
varhandle::rm.all.but(keep = c("degs_4vs2", "protein_coding"))
varhandle::save.var(path = here("analysis"), varlist = c("degs_4vs2", "protein_coding"))
```
