...
weightedSetCover <- function(cur_config, geneset_ids, geneset_info, output_dir, n_threads){
  ...
  mc_results <- parallel::mclapply(all_set_names, function(cur_name, cur_res, geneset_ids, geneset_info){
  ...
  for(i in seq(top_k)){
    ...
    if(nrow(candidates) > 0){  # seq(0) = 1 0
      mc_results <- parallel::mclapply(seq(nrow(candidates)), function(row, candidates, cur_res, geneset_ids, geneset_info){
