...
bind_rows(brca_wp, coadread_wp, prad_wp, luad_wp) %>% distinct(pathway_name) %>% count()
  # 294; There are 294 unique enriched WikiPathways pathways
```
...
varhandle::rm.all.but(keep = c("brca*", "prad*", "luad*", "coadread*", "common_all"), keep_functions = F)
varhandle::save.var(path = here("analysis"), newdirtag = "NEArender_results")
```
