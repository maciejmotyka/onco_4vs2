...
```{r warning=F}
degs_breast <- bind_rows(breast_up, breast_down)
degs_colorectal <- bind_rows(colorectal_up, colorectal_down)
degs_lung <- bind_rows(lung_up, lung_down)
degs_prostate <- bind_rows(prostate_up, prostate_down)

venn_all_degs <- gplots::venn(
  list("BRCA" = degs_breast,
       "COADREAD" = degs_colorectal,
       "LUAD" = degs_lung,
       "PRAD" = degs_prostate))

common_degs_ensg <- attr(venn_all_degs, "intersections")$`BRCA:COADREAD:LUAD:PRAD`
```

## Place the Venn diagrams on one figure
```{r}
# grDevices::pdf(file="saving_plot.pdf")
graphics::par(mfrow = c(2,2), mai = c(0,0,0.2,0), cex = 0.7, oma = c(0, 0, 4, 0))
...
graphics::par(mar = c(0,0,1,0), fig=c(0,0.7,0,0))
graphics::plot(venn_all_degs)
graphics::title(main = "all DEGs")
graphics::mtext("Overlap between differentially expressed genes", outer = T, cex = 1, padj = -2)
# grDevices::dev.off()
```
...
common_degs_symbols <- AnnotationDbi::mapIds(org.Hs.eg.db, keys = common_degs_ensg, keytype = "ENSEMBL", column = "SYMBOL", multiVals = "first")
```
...
cgc %>% filter_at("Gene Symbol", any_vars(. %in% common_degs_symbols)) # CD74, PBX1
```
