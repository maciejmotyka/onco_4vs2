...
filterCancer <- function(genes, cancer_data){
  cancer_data %>%
  dplyr::select(gene:not_detected) %>% 
  ...
  mutate(n_samples = rowSums(dplyr::select(., high:not_detected)))
}
...
```{r}
degs_breast <-
  list("up" = breast_up, "down" = breast_down) %>% bind_rows(.id = "change")
degs_colorectal <-
  list("up" = colorectal_up, "down" = colorectal_down) %>% bind_rows(.id = "change")
degs_lung <-
  list("up" = lung_up, "down" = lung_down) %>% bind_rows(.id = "change")
degs_prostate <-
  list("up" = prostate_up, "down" = prostate_down) %>% bind_rows(.id = "change")

all_degs_HPA <- list("BRCA" = degs_breast,
...
## Clean up
```{r}
varhandle::rm.all.but(keep = c("all_degs_HPA", "protein_coding"), keep_functions = F)
