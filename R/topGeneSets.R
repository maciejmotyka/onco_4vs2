...
topGeneSets <- function(cur_config, geneset_ids, geneset_info, output_dir, prefix=""){
  ...
  for(i in seq_along(geneset_info)){
	...
	# select up to top_num best scoring pathways
	# select top 50
  ...
  `%>%` <- magrittr::`%>%`
	...
	my_plot <- ggplot2::ggplot(plot.df, aes(x = name, y = score, fill = myclass)) +
		...
		theme(axis.text.y = element_blank(),
 				 ...
 				 scale_y_continuous(expression(italic("score")),
														breaks = waiver(), limits = c(min.y, max.y)) # breaks = seq(min.y, max.y, 10)
